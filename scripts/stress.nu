...
def "main import" [base: string = "http://localhost:3080"] {
  sudo docker pull oitc/modbus-server:1.3.1
  sudo docker pull frrouting/frr:v8.4.0
  open --raw (nix build $"github:altibiz/ozds#docker" --print-out-paths --no-link) | sudo docker load
  open --raw (nix build $"github:altibiz/pidgeon#docker" --print-out-paths --no-link) | sudo docker load
